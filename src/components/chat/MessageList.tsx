...
export const MessageList: React.FC<MessageListProps> = ({ messages }) => {
    ...
    return (
        <div className="flex-1 overflow-y-auto p-4 bg-[#F5F5F5]">
            ...
            {messages.map((msg) => (
                <div key={msg.id} className={`flex mb-4 gap-2 ${msg.isMe ? 'flex-row-reverse' : ''}`}>
                    <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0">
                        <img src={msg.avatar} alt={msg.sender} className="w-full h-full object-cover" />
                    </div>
                    <div className={`flex flex-col max-w-[70%] relative ${msg.isMe ? 'items-end' : 'items-start'}`}>
                        {!msg.isMe && <span className="text-[12px] text-[#999999] mb-1 ml-1">{msg.sender}</span>}
                        <div
                            className={`px-3 py-2 relative break-words text-[16px] leading-relaxed ${msg.isEggWord
                                    ? 'pl-[50px] rounded-[50px] min-h-[40px] flex items-center overflow-visible text-white shadow-sm'
                                    : `${msg.isMe ? 'bg-[#0099FF] text-white rounded-tr-[2px] rounded-tl-[12px] rounded-bl-[12px] rounded-br-[12px]' : 'bg-white text-[#333333] rounded-tl-[2px] rounded-tr-[12px] rounded-bl-[12px] rounded-br-[12px]'}`
                                }`}
                            style={
                                msg.isEggWord
                                    ? {
                            ...
                            {msg.isEggWord && (
                                <>
                                    <div className="absolute w-[48px] h-[54px] -top-[20px] -left-[15px] flex justify-center items-center z-20">
                                        <svg width="100%" height="100%" viewBox="0 0 54 60" fill="none" xmlns="http://www.w3.org/2000/svg" className="absolute top-0 left-0 drop-shadow-sm">
                                        ...
                                        <div className="z-20 text-[28px] mb-[5px]">{bubbleSticker}</div>
                                    </div>
