...
import { Mic, Image as ImageIcon, Camera, Smile, PlusCircle, Phone, Video, MapPin, Folder, Bookmark, CreditCard, Share2, Scissors } from 'lucide-react';
    ...
    return (
        <div className="flex flex-col w-full bg-[#F7F7F7] border-t border-[#E5E5E5] pb-safe">
            ...
            <div className="flex items-center px-3 py-2 gap-3">
                ...
                />
                <button
                    className={`bg-[#0099FF] text-white px-4 py-1.5 rounded-[6px] text-[14px] font-medium transition-opacity duration-200 whitespace-nowrap ${hasText ? 'opacity-100' : 'opacity-60'}`}
                    onClick={handleSend}
                    disabled={!hasText}
                >
                    发送
            ...
            <div className="flex items-center justify-between px-6 pb-2">
                ...
                <button
                    className={`p-1 transition-colors ${isPanelOpen ? 'text-[#0099FF]' : 'text-[#1A1A1A]'}`}
                    onClick={togglePanel}
                >
...
const PanelItem: React.FC<{ icon: React.ReactNode, label: string }> = ({ icon, label }) => (
